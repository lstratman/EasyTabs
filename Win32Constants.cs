namespace Stratman.Windows.Forms.TitleBarTabs
{
	...
	internal static class Win32Constants
	{
		...
		/// <summary>
		/// The source surface can be assumed to be in a pre-multiplied alpha 32bpp "BGRA" format; that is, the surface type is BMF_32BPP and the palette type 
		/// is BI_RGB. The alpha component is an integer in the range of [0,255], where 0 is completely transparent and 255 is completely opaque.
		/// </summary>
		...
		/// <summary>
		/// All descendants of a window get bottom-to-top painting order using double-buffering. Bottom-to-top painting order allows a descendent window to 
		/// have translucency (alpha) and transparency (color-key) effects.
		/// </summary>
		public static int WS_EX_COMPOSITED = 0x02000000;

		/// <summary>
		/// Specifies that a window created with this style is to be transparent. That is, any windows that are beneath the window are not obscured by the 
		/// window. A window created with this style receives WM_PAINT messages only after all sibling windows beneath it have been updated.
		/// </summary>
		public static int WS_EX_TRANSPARENT = 0x20;

		/// <summary>
		/// The window is a layered window. Note that this cannot be used for child windows. Also, this cannot be used if the window has a class style of 
		/// either CS_OWNDC or CS_CLASSDC.
		/// </summary>
		public static int WS_EX_LAYERED = 0x00080000;

		/// <summary>
		/// A top-level window created with this style does not become the foreground window when the user clicks it. The system does not bring this window to 
